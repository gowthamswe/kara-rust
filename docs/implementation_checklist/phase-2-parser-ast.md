...
- [ ] **Top-level `pub const X: T = lit;` not visible from function bodies (`Undefined variable`).** Surfaced 2026-05-08 during slice 6 (Parallax-lite microbenchmark workload): a top-level `pub const WORK: i64 = 50000000;` declared in `examples/parallax_lite/src/workload.kara` and referenced from a function body in the same module raises `Undefined variable 'WORK'`. Workaround applied in slice 6: hardcode the literal inline in each driver function (loses tunability for benchmark sweeps). AST and parser surface exists (`ast.rs:128 Item::ConstDecl(ConstDecl)`, `ast.rs:563 pub struct ConstDecl`, `parser.rs:1758 parse_const_decl`, `parser.rs:264 Item::ConstDecl(decl)`); resolver has `collect_const` (`resolver.rs:1205`) and `resolve_const_decl` (`resolver.rs:1883`) — so the registration mechanism is in place. The bug is somewhere between resolver registration and downstream lookup at function-body call sites. **Likely root-cause hypotheses (verify before fixing):** (a) `collect_const` registers the symbol but only at file scope, and function-body scope resolution doesn't inherit module-level consts; (b) typechecker doesn't infer the type of a const-name reference (no `expr_types` entry); (c) lowering or codegen doesn't have a handler for `Path([X])` resolving to a const decl. **Reproducer:** `pub const WORK: i64 = 100;\nfn use_work() -> i64 { WORK }` should typecheck and codegen; today the resolver fires `Undefined variable 'WORK'`. Phase target: Phase 2 / Phase 3.1 (resolver) — investigation determines exactly where the lookup chain breaks. No P1 carve-out — small fix once the root cause is pinned.
