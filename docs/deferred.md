...
**Why deferred:** Empirical motivation is concentrated in a single sort-saturated workload — the LeetCode #1665 bench (`kata-katas/leetcode/1601-1700/1665-.../bench/`, 2026-05-12) shows ~1.4× vs hand-tuned Rust with the remaining gap at the FFI hop. Outside sort-dominated code, the gap hasn't been measured. The fix involves real path choices; the originally-hypothesised cheap option (Path A: runtime-side LTO + `#[inline]`) was empirically falsified on 2026-05-12 — see *Path A experiment* below. The actually-cheap path (Path A+) requires a small codegen change and is untested.

**Promotion gate:** Promote Path A+ or Path B to P1 (or P0 v1.x) when post-v1 user data shows ≥2 distinct non-synthetic workload classes where Karac's `Vec.sort_by` (or any runtime-helper-with-closure call) leaves a >1.3× perf gap. The trigger is *frequency in real code*, not theoretical coverage. The original "if the cheap LTO experiment closes the gap, no promotion needed" criterion has been removed — that experiment was tried and didn't close the gap (see Path A entry below).
...
- **Path A+ — fat LTO + karac emits bitcode.** What Path A *needed* to be. ~10–20 line codegen change: in `compile_to_object_with_options` (`src/codegen.rs:175`), call `module.write_bitcode_to_path(...)` alongside (or instead of) `write_to_file(FileType::Object, ...)`; in `link_executable_impl` (`src/codegen.rs:221`), pass `-flto` to `cc` and feed the bitcode file into the link line. With caller-side bitcode present *and* the runtime archive at `lto = "fat"` (already in place), link-time LTO can specialize `karac_vec_sort_by` per call site by inlining the constant `&thunk_fn` value across the `extern "C" fn` parameter, then inlining the comparator body across the indirect-call hop. Untested empirically; would close the gap if LLVM cooperates as expected. Tradeoffs not yet measured: bitcode emission adds time to every `karac build`, `-flto` adds time to every link, and the karac compile-time advantage over `rustc -O` (currently ~2×) may shrink. If link cost is meaningful, gate behind `karac build --release` rather than defaulting.
- **Path B — monomorphized runtime helpers emitted per-call-site by codegen.** Codegen emits `__karac_vec_sort_by_<elem_mangle>` shape with the comparator inlined statically. Sort algorithm body still comes from the runtime crate (don't fork pdqsort / TimSort) but is duplicated per element type. Larger codegen scope; matches Rust's `slice::sort_by` shape. The fallback if Path A+ also disappoints empirically.
...
**Cross-reference:** `runtime/src/lib.rs::karac_vec_sort_by`; `src/codegen.rs::emit_sort_by_inline_thunk`, `compile_to_object_with_options`, `link_executable_impl`; `kata-katas/leetcode/1601-1700/1665-.../bench/` (workload that surfaced the gap, 2026-05-12; Path A experiment also 2026-05-12); workspace `Cargo.toml § [profile.release]` (fat LTO bump, kept for the binary-size side effect even though it didn't close the runtime gap).
