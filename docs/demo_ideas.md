...
- **Closing the Kāra-vs-Rust gap (conditional on F3 numbers).** If the 4-way table shows a material Kāra-vs-Rust delta — *and only then* — the closure path is, in priority order: (1) **borrowed accessors** — `req.path() -> ref StringSlice` returning a view into hyper's request buffer instead of allocating a fresh `String` per call (today's F2 lowering re-allocates per call via `karac_runtime_http_request_path` at `runtime/src/lib.rs:1828`); requires threading hyper's request lifetime into Kāra's borrow checker across the FFI boundary and promoting `Request` from opaque heap pointer to a typed handle whose lifetime is the borrow root. (2) **Inline the handler trampoline** — emit the shim body at the handler call site instead of as a separate symbol; trades binary size for a saved call frame. (3) **`#[repr(C)]`-compatible `Request`** — let user code manipulate the same memory hyper handed the runtime, no packing layer at all; largest design surface (Kāra's value-type ABI would need a story for "this struct is laid out by an external producer"). (1) alone likely closes most of the gap; (2) and (3) are diminishing returns. Per F3, measure first — don't pre-commit to this work before the data justifies it; if the measured gap is small or inside Rust's noise floor, none of these are worth the budget.
