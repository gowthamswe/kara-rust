...
use helpers::{
    ...
    impl_target_name, make_impl_method_function, map_kv_type_exprs, match_with_provider_call,
    method_self_is_value, set_inner_type_expr, slice_inner_type_expr, vec_inner_type_expr,
};
use state::{
    AssertedIndexBound, CleanupAction, EnumDropKind, EnumLayout, LoopFrame, MapMonoMethods,
    ReturnSlot, SetOpFilter, SharedTypeInfo, SliceSource, SoaGroup, SoaLayout, SpawnSiteRecord,
    VarSlot,
};
...
impl<'ctx> Codegen<'ctx> {
    ...
    fn resolve_slice_source(&mut self, expr: &Expr) -> Option<SliceSource<'ctx>> {
    ...
    fn compile_slice_pattern_condition(
        &mut self,
        prefix: &[Pattern],
